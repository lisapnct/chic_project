...
// get all designer's projects
router.get("/designer", async (req, res, next) => {
  try {
    const apiRes = await Project.find({
...
router.patch("/:id/contributions", async (req, res, next) => {
  try {
    ...
    if (foundProjectAlr.length > 0) {
      const contribution = foundProjectAlr[0].contributors.find(
        (contributor) => contributor.id_user == currentUserId
      );
      contribution.contributed_materials.find(
    ...
    if (foundProjectNve.length > 0 && foundProjectAlr.length > 0) {
      const material = foundProjectAlr[0].materials.find(
        (mat) => mat.fabric_type === fabric_type
      );
      material.collected_quantity += Number(quantity);
      if (material.collected_quantity >= material.required_quantity)
        material.fullyCollected = true;
      if (
        foundProjectAlr[0].materials.every((elm) => elm.fullyCollected === true)
      )
        foundProjectAlr[0].isSuccess = true;
    ...
    if (foundProjectNve.length > 0 && foundProjectAlr.length === 0) {
      const material = foundProjectNve[0].materials.find(
        (mat) => mat.fabric_type === fabric_type
      );
      material.collected_quantity += Number(quantity);
      if (material.collected_quantity === material.required_quantity)
        material.fullyCollected = true;
      if (
        foundProjectNve[0].materials.every((elm) => elm.fullyCollected === true)
      )
        foundProjectNve[0].isSuccess = true;

      const contribution = foundProjectNve[0].contributors.find(
        (contributor) => contributor.id_user == currentUserId
      );
      contribution.contributed_materials.push({
    ...
    if (
      foundProjectNve.length === 0 &&
      foundProjectAlr.length === 0 &&
      foundProject.length > 0
    ) {
      const material = foundProject[0].materials.find(
        (mat) => mat.fabric_type === fabric_type
      );
      material.collected_quantity += Number(quantity);
      if (material.collected_quantity >= material.required_quantity)
        material.fullyCollected = true;
      if (foundProject[0].materials.every((elm) => elm.fullyCollected === true))
        foundProject[0].isSuccess = true;

      foundProject[0].contributors.push({
        id_user: currentUserId,
        contributed_materials: [
          { fabric_type: fabric_type, quantity: Number(quantity) },
        ],
      });
    ...
    const apiRes = await Project.findOneAndUpdate(search, query, {
      new: true,
    })
      .populate("contributors.id_user", "profilePicture userName")
      .populate("creator", "profilePicture userName");
